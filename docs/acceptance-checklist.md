...
| # | Question | Status | Evidence |
|---|----------|--------|----------|
| M5 | Can the application display all items showing title, amount, month, and income/expense type? | ✅ | `PrintList()` in `Program.cs` — menu option 1 (sub-menu: All / Expenses / Incomes) |
...
| # | Question | Status | Evidence |
|---|----------|--------|----------|
| M11 | Does the application provide a text-based menu that makes add, list, sort, filter, edit, remove, save, and quit discoverable? | ✅ | Menu printed before every input; options 1–6 and 0 cover all actions |
| M12 | Can a user add an income item through the CLI? | ✅ | Option 2 → sub-prompt `1=Income` → `AddItem(collection, ItemType.Income)` |
| M13 | Can a user add an expense item through the CLI? | ✅ | Option 2 → sub-prompt `2=Expense` → `AddItem(collection, ItemType.Expense)` |
| M14 | Can a user edit an existing item (at least one field)? | ✅ | Option 3 → `1=Edit` → `EditItem()` — title, amount, month editable; Enter keeps current value |
| M15 | Can a user remove an existing item? | ✅ | Option 3 → `2=Remove` → `RemoveItem()`; removes by `Guid` |
| M16 | Can a user quit the application via a menu option? | ✅ | Option `0` saves and exits |
...
|---|----------|--------|----------|--------------------------|
| O1 | Does the application display a running total or balance (income minus expenses)? | ✅ | Balance shown in menu header before every prompt **and** in summary line below every list | Uses `decimal` arithmetic; empty collection shows `+0.00` |
